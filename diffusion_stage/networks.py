...
class MLPblock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.w_embed:
            x = inputs[0]  # (batch, 196, 1024)
            embed = inputs[1]  # (batch, 1, 512)
            x = self.conct(x) + self.emb_fc(self.act(embed))  # (batch, 196, 512)
        else:
            x = inputs

        x_ = self.norm0(x)
        x_ = self.fc0(x_)  # (batch, seq*8, 512)
        x_ = self.act(x_)
        x = x + x_
        ...
        x = x + x_  # (batch, 196, 512)

        if self.w_embed:
            # x: (batch, 196, 512)    embed:(batch, 1, 512)
            return x, embed
        else:
            return x
...
class DiffMLP(nn.Module):
    ...
    def forward(self, motion_input, embed):
        # motion_input:(batch, 196, 1024)   embed:(batch, 1, 512)
...
class PureMLP(nn.Module):
    ...
    def forward(self, motion_input):
        # motion_input:(batch, 196, 54)
        motion_feats = self.input_fc(motion_input)  # (batch, 196, 512)
        motion_feats = self.motion_mlp(motion_feats)  # (batch, 196, 512)
        motion_feats = self.output_fc(motion_feats)  # (batch, 196, 132)

        return motion_feats

    def forward_feat(self, motion_input):
        # motion_input:(batch, 196, 54)
        motion_feats = self.input_fc(motion_input)  # (batch, 196, 512)
        motion_feats = self.motion_mlp(motion_feats)  # (batch, 196, 512)
        return motion_feats
